...
All notable changes to the Okx Wasmy extension will be documented in this file.
...
- Closed [#15](https://github.com/spoo-bar/okx-wasmy/issues/15) - Can now send funds while executing a contract and while initializing a contract 💰
...
- Added new setting `okxwasmy.openTxExplorerInVscode`, if set to true, on a successful tx execution, instead of a notification, the tx explorer link will open within vscode in the simple browser. (only works if txExplorerLink is set as well for the chain config)
...
- On `okx-wasmy.generateSchema`, the json schema is set into the workspace settings so that any json file can get code completion from the query or execute msgs. (Thanks [Callum](https://github.com/Callum-A) for feature suggestion)
...
- Added `okxwasmy.chains.chainEnvironment` to chain config which stores if the chain is localnet, testnet or mainnet.
...
- Added command `okx-wasmy.refreshAccount` to refresh the account view in the sidebar.
...
- `okxwasmy.chainConfigName` setting deprecated and the config will now be set using the extension command `okx-wasmy.reloadConfig`
...
- Added [#10](https://github.com/spoo-bar/okx-wasmy/issues/10) - Developers can add small notes and comments to their imported contracts. The added notes are shown on hovering over the contracts in the sidebar contract view. The notes supports markdown. Note : The added notes are only for the dev view, this info is not stored on chain.
- Added [#11](https://github.com/spoo-bar/okx-wasmy/issues/11) - Block explorer link added to the account view. If set up, the icon opens the account in a block explorer in the default browser
- Added Osmosis testnet and localnet settings into default populated chain configs

### Changed

- Fixed [#5](https://github.com/spoo-bar/okx-wasmy/issues/5) - When a contract is imported, it is automatically associated with the chain config it was imported with. 
...
- Added "Initialize + Import" option. This automatically imports the contract into Okx Wasmy after it has been initialized
...
- Fixed [#9](https://github.com/spoo-bar/okx-wasmy/issues/9) - Default key binding for `okx-wasmy.history` changed from `ctrl+shift+h` to `ctrl+shift+a`.
...
- Added command `okx-wasmy.history` - When enabled, the latest `okxwasmy.maxHistoryStored` number of queries are stored and are accessible in history view for easy re-execution. History opens in a new webview as a tab. This command also has a menu icon on the query and execute views.
...
- Fixed [#8](https://github.com/spoo-bar/okx-wasmy/issues/8) - Initiation of contract was failing with "Input is not an integer" due to codeId not being parsed into number.
...
- Implemented [#3](https://github.com/spoo-bar/okx-wasmy/issues/3) & [#4](https://github.com/spoo-bar/okx-wasmy/issues/4) - The smart contracts in the Contract view can now be sorted alphabetically, or with CodeID, or in imported order
...
- Added commands for the following (with keybindings):
    - __okx-wasmy.setupDevEnv__ - Setup dev environment and install all dependencies
    - __okx-wasmy.build__ - Build the project - `ctrl+shift+b`
    - __okx-wasmy.runUnitTests__ - Run unit tests - `ctrl+shift+t`
    - __okx-wasmy.optimizeContract__ - Run contract optimizer using the docker image (Needs docker running) - `ctrl+shift+o`
    - __okx-wasmy.generateSchema__ - Generate json-schema using the rust schema files - `ctrl+shift+g`
    - __okx-wasmy.upload__ - Upload wasm file to chain - `ctrl+shift+u`
- Added placeholder text for all the webviews (query, execute, sign)
- Implemented [#7](https://github.com/spoo-bar/okx-wasmy/issues/7) - Added setting so user can choose to output the tx and query responses to the output channel or to new file
- Added snippets to easily add new query and msg functions, and to check if the sender is the smart contract admin.

### Changed

- Implemented [#2](https://github.com/spoo-bar/okx-wasmy/issues/2) - Show faucet icon for account only when faucet has been set up for that chain in the settings
...
- Fixed [#1](https://github.com/spoo-bar/okx-wasmy/issues/1) - Using contract address as id (instead of codeId) for the rendered item in the Contract view
